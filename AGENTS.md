...
> ## Repository scope override --- READ FIRST
>
> **This repository is NOT a WordPress plugin.** It is a standalone, client-side
> TypeScript npm package (`@starisian/3iatlas-multilingual-input`) consumed by
> WordPad, Dictionary, Games, RLC, and non-WordPress products.
>
> The following parts of this document DO NOT APPLY here and must not be
> reintroduced: PHP, Composer, PHPCS, PHPStan, PHPUnit, the `mu-plugins/`
> loader, WordPress version policy, the WordPress JS standard
> (`@wordpress/data`, `@wordpress/i18n`, `@wordpress/api-fetch`,
> `@wordpress/scripts`), the Abilities API, and the Sirus mandatory integration
> pattern. There is no PHP, database, or server in this repository.
>
> What still applies: TypeScript 5 strict, Node 20 LTS, Ubuntu 24 CI, pnpm with
> `pnpm-lock.yaml` as the only lockfile, named exports only, no `any`, no `var`,
> `ai_manifest.json` symbol governance, the accessibility and bandwidth rules,
> and the Git and PR rules.
>
> Repository-specific tooling, which replaces the table below:
>
> | Tool                 | Config file                | Command              |
> | -------------------- | -------------------------- | -------------------- |
> | ESLint (flat config) | `eslint.config.js`         | `pnpm run lint`      |
> | Prettier             | `.prettierrc.json`         | `pnpm run format`    |
> | TypeScript           | `tsconfig.json`            | `pnpm run typecheck` |
> | Jest                 | `jest.config.js`           | `pnpm test`          |
> | tsup                 | per-package `package.json` | `pnpm run build`     |
>
> Note: ESLint 9+ reads flat config only and ignores `.eslintrc.*`, which is why
> this repository uses `eslint.config.js`.
>
> [`TECHNICAL_SPEC.md`](TECHNICAL_SPEC.md) governs product scope and takes
> precedence over this document wherever the two disagree.
